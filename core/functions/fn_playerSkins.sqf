...
switch(playerSide) do {
	...
	case west: {
		...
		if(uniform player == "U_Rangemaster" && cop_level > 5) then {
			player setObjectTextureGlobal [0, "textures\PK.png"];
		};
	};
	case independent: {
		if(uniform player == "U_Rangemaster") then {
			player setObjectTextureGlobal [0, "textures\thw_uniform.jpg"];
		};
	};
};
